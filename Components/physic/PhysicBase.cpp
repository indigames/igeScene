...
namespace ige::scene
{
    ...
    void PhysicBase::createBody()
    {
        ...
        if(m_bIsTrigger) addCollisionFlag(btCollisionObject::CF_NO_CONTACT_RESPONSE);
        if(m_bIsEnabled) activate();
    }
    ...
    void PhysicBase::applyInertia()
    {
        ...
        if (m_mass != 0.0f) m_shape->calculateLocalInertia(m_mass, inertia);
